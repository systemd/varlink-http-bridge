...
presented as `Authorization: Bearer <token>`. Select it with `--auth=jwt`
(it combines with other mechanisms, e.g. `--auth=ssh,jwt`). This is vendor-neutral:
...
(its `aud` is shared, not per-caller). So `--auth=jwt` refuses to start until
...
```
--auth=jwt                  select JWT bearer auth (combinable, e.g. --auth=ssh,jwt)
  ...
  drive-node:
    ...
    steps:
      - run: |
          ...
          VARLINK_JWT="$TOKEN" \
          VARLINK_BRIDGE_URL=https://node-1:1031/ws/services/io.systemd.Hostname \
            varlinkctl call exec:/usr/libexec/varlinkctl-http \
            io.systemd.Hostname.Describe '{}'
```
...
`systemd.exec(5)` and `systemd.system-credentials(7)`). Credentials configure
the mechanism but do not select it: `--auth=jwt` is still required, and the
bridge warns at startup about JWT credentials it finds while jwt is not selected.
