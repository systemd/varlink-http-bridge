...
/// JWT credentials present in `dir` that this configuration will not read,
/// each with the reason, for the startup warning.
pub(crate) fn unread_credentials(
    dir: &std::path::Path,
    cli: &crate::JwtCliOptions,
    selected: bool,
) -> Vec<String> {
    let creds = CredentialsLoader::from_dir(dir);
    [
        (ISSUER_CREDENTIAL, cli.issuer.is_some(), "--issuer="),
        (AUDIENCE_CREDENTIAL, cli.audience.is_some(), "--audience="),
        (JWKS_CREDENTIAL, cli.issuer_jwks.is_some(), "--issuer-jwks="),
        (
            REQUIRE_CLAIMS_CREDENTIAL,
            !cli.require_claims.is_empty(),
            "--require-claim=",
        ),
    ]
    .into_iter()
    .filter(|(name, _, _)| creds.path(name).is_some())
    .filter_map(|(name, explicit, flag)| {
        if !selected {
            Some(format!("{name} (pass --auth=jwt to use it)"))
        } else if explicit {
            Some(format!("{name} ({flag} takes priority)"))
        } else {
            None
        }
    })
