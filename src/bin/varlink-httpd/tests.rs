...
async fn test_jsonseq_userdb_get_user_record_more() {
    ...
    let res = client
        .post(format!(
            "http://{}/call/io.systemd.UserDatabase.GetUserRecord?service=io.systemd.Multiplexer&more=true",
            server.addr,
        ))
...
fn cli_looking_up_credentials(
    ...
    crate::BridgeCli {
        ...
        api_keys: None,
        auth: auth.to_vec(),
        jwt: crate::JwtCliOptions::default(),
        insecure,
    }
}
...
fn build_authenticators_in(
    ...
    crate::build_authenticators(
        auth,
        insecure,
        require_mtls,
        None,
        None,
        &crate::JwtCliOptions::default(),
        None,
        etc_root,
    )
}
...
fn test_call_mode_from_params() {
    ...
    fn oneway(oneway: &str, more: Option<&str>) -> HashMap<String, String> {
        let mut p = params(more);
        p.insert("oneway".to_string(), oneway.to_string());
        p
    }
    assert_eq!(
        CallMode::from_params(&oneway("true", None)).unwrap(),
        CallMode::Oneway
    );
    assert_eq!(
        CallMode::from_params(&oneway("false", Some("true"))).unwrap(),
        CallMode::More
    );
    assert!(CallMode::from_params(&oneway("true", Some("true"))).is_err());
    assert!(CallMode::from_params(&oneway("yes", None)).is_err());
}
...
fn test_idl_to_openapi() {
    ...
    assert_eq!(
        ...
        "WatchItems should advertise ?more= but not ?oneway="
    );
    assert!(
...
async fn test_integration_call_oneway() {
    ...
    let res = client
        .post(format!(
            "http://{}/call/org.varlink.service.GetInfo?service=io.systemd.Journal",
            server.addr,
        ))
        .json(&json!({}))
        .send()
...
async fn test_integration_call_oneway_rejects_more() {
    ...
    let res = client
        .post(format!(
            "http://{}/call/io.systemd.Hostname.Describe?oneway=true&more=true",
            server.addr,
        ))
        .json(&json!({}))
        .send()
...
mod apikey_tests {
    ...
    /// A credentials directory that cannot be listed must not look like
    /// "all credentials were removed" and revoke their keys.
    #[test]
    fn test_api_key_auth_unreadable_credentials_dir_keeps_cached_keys() {
        let root = tempfile::tempdir().unwrap();
        let creds_dir = root.path().join("credentials");
        ...
        let auth = create_api_key_authenticator(None, Some(&creds_dir), root.path())
            ...
            .unwrap();
        let header = format!("Bearer {key}");
        check_request(&auth, "GET", "/sockets", Some(&header), None, None).unwrap();

        // a regular file in place of the directory makes read_dir() fail
        // with ENOTDIR, which is not the "no credentials" NotFound case
        std::fs::remove_dir_all(&creds_dir).unwrap();
        std::fs::write(&creds_dir, b"").unwrap();
        check_request(&auth, "GET", "/sockets", Some(&header), None, None)
            .expect("an enumeration error must keep the cached keys");
        assert_eq!(auth.key_count(), 1);
    }

    /// One source that cannot be stat()ed must not stop the others from
    /// being refreshed.
    #[test]
    fn test_api_key_auth_broken_source_does_not_block_reload() {
        let dir = tempfile::tempdir().unwrap();
        let good = dir.path().join("good/api-keys");
        let key = crate::auth_api_key::generate_api_key();
        append_api_key(&good, &key, None).unwrap();
        // does not exist yet, so startup treats it as a tracked absence
        let broken = dir.path().join("broken/api-keys");
        ...
        // a regular file where the parent directory should be makes stat()
        // fail with ENOTDIR from now on
        std::fs::write(broken.parent().unwrap(), b"").unwrap();
        let late_key = crate::auth_api_key::generate_api_key();
        append_api_key(&good, &late_key, None).unwrap();
        let header = format!("Bearer {late_key}");
        check_request(&auth, "GET", "/sockets", Some(&header), None, None)
            .expect("the readable source must still be reloaded");
        assert_eq!(auth.key_count(), 2);
    }
